...
function ComboboxTrigger({
	...
	return (
		<ComboboxPrimitive.Trigger className={cn("w-full", className)} {...props} />
	);
}
function ComboboxInput({ className, ...props }: ComboboxPrimitive.Input.Props) {
	return (
		<ComboboxPrimitive.Input
			className={cn("w-full border-0 bg-transparent outline-none", className)}
			{...props}
		/>
	);
}
...
function ComboboxItem({ className, ...props }: ComboboxPrimitive.Item.Props) {
	return (
		<ComboboxPrimitive.Item
			className={cn(
				"flex w-full cursor-default items-center rounded-lg px-3 py-2 text-left text-sm outline-none data-highlighted:bg-accent",
				className,
			)}
			{...props}
		/>
